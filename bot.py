...
# Cấu hình log để theo dõi trên Render
logging.basicConfig(
...
# Lấy cấu hình bảo mật từ Biến môi trường
...
def clear_system_cached_files():
    """Quét và xóa bỏ toàn bộ file json tạm và file excel đã chuyển đổi còn sót lại trên đĩa cứng"""
    logger.info("🧹 Đang kích hoạt lệnh quét dọn hệ thống...")
    count = 0
    # Tìm tất cả các file tạm bắt đầu bằng temp_ hoặc kết thúc bằng _converted.xlsx
...
def convert_json_to_styled_excel_heavy(json_filepath, excel_filepath, queue, loop):
    ...
    # Ghi dữ liệu học sinh & Tính toán tiến trình
    ...
    for idx, (sbd, scores) in enumerate(students_data.items(), start=1):
        ...
        # Cứ tăng lên 10% thì báo tiến độ 1 lần
        if total_students > 0:
    ...
    logger.info("✨ Ghi file Excel hoàn tất!")

# --- CÁC HÀM XỬ LÝ LỆNH BOT TELEGRAM ---
async def start_command(update: Update, context):
    """Xử lý lệnh /start - Giải phóng bộ nhớ và dọn rác hệ thống"""
    # Chạy hàm xóa file rác trong luồng Executor để không ảnh hưởng hiệu năng bot
    ...
    if deleted_files_count > 0:
        msg += f"🧹 Đã dọn dẹp tận gốc {deleted_files_count} tệp tin tạm còn sót lại.\n"
    else:
        msg += "✨ Không có tệp tin rác nào được tìm thấy. Hệ thống sạch 100%.\n"
        
    msg += "\nBộ nhớ RAM hiện tại trống hoàn toàn. Hãy gửi file `.json` mới để bắt đầu xử lý siêu tốc!"
...
async def handle_document(update: Update, context):
    ...
    try:
        ...
        convert_task = loop.run_in_executor(
            None, 
            convert_json_to_styled_excel_heavy, 
            json_path, 
            excel_path, 
            queue, 
            loop
        ...
        await update.message.reply_text("💥 Đã xảy ra lỗi trong quá trình xử lý file thời gian thực.")
        
    finally:
        # Cơ chế dọn dẹp sau lượt chạy (Luôn chạy bất kể thành công hay thất bại)
...
@app.get("/")
def health_check():
    return {"status": "online", "auto_cleanup": "active"}
